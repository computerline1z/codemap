...
class Codemap(object):
    def __init__(self):
        ...
        if sysname == 'Darwin' or sysname == 'Linux':
            ...
            self.homedir = '%s\\codemap\\' % idaapi.idadir("plugins")
    ...
    def init_codemap(self):
        ...
        self.interaction = open(self.homedir + 'ui/interaction.js', 'rb').read()
...
class X86(BasicArchitecture):
    def __init__(self):
        self.db_fmt = '{0} INT8, m_{0} VARCHAR(2048)'
        self.name = 'x86'
        self.reg_list = 'eip eax ebx ecx edx esi edi ebp esp arg1 arg2 arg3 arg4'.split()
        self.reg = {}
        self.memory = {}
        for i in self.reg_list:
            self.reg[i] = 0 # int
...
class X64(BasicArchitecture):
    def __init__(self):
        self.name = 'x64'
        self.db_fmt = '{0} VARCHAR(32), m_{0} VARCHAR(2048)'
